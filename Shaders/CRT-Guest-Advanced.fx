...
texture GUEST_T1{Width=Resolution_X;Height=Resolution_Y ;Format=RGBA16F;};
sampler GUEST_S1{Texture=GUEST_T1;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T2{Width=Resolution_X;Height=Resolution_Y ;Format=RGBA16F;};
sampler GUEST_S2{Texture=GUEST_T2;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T3{Width=800.00000000;Height=600.00000000 ;Format=RGBA16F;};
...
texture GUEST_T4{Width=800.00000000;Height=600.00000000 ;Format=RGBA16F;};
...
texture GUEST_T7{Width=BUFFER_WIDTH;Height=BUFFER_HEIGHT;Format=RGBA16F;};
...
float3 fetch_pixel(float2 coord)
{
	...
	float2 d2=dx-dy;
	float sum=15.0;
	float3 result=3.0*COMPAT_TEXTURE(GUEST_S0,coord).rgb+2.0*COMPAT_TEXTURE(GUEST_S0,coord+dx).rgb+2.0*COMPAT_TEXTURE(GUEST_S0,coord-dx).rgb+
	2.0*COMPAT_TEXTURE(GUEST_S0,coord+dy).rgb+2.0*COMPAT_TEXTURE(GUEST_S0,coord-dy).rgb+COMPAT_TEXTURE(GUEST_S0,coord+d1).rgb+
	COMPAT_TEXTURE(GUEST_S0,coord-d1).rgb+COMPAT_TEXTURE(GUEST_S0,coord+d2).rgb+COMPAT_TEXTURE(GUEST_S0,coord-d2).rgb;
	return result/sum;
}
...
void bring_pixel(inout float3 c,inout float3 b,inout float3 g,float2 coord,float2 boord)
{
	...
	float r1=COMPAT_TEXTURE(GUEST_S7,coord+rc).r;
	float g1=COMPAT_TEXTURE(GUEST_S7,coord+gc).g;
	float b1=COMPAT_TEXTURE(GUEST_S7,coord+bc).b;
	float ds=decons;
	...
	r1=COMPAT_TEXTURE(GUEST_S4,boord+rc).r;
	g1=COMPAT_TEXTURE(GUEST_S4,boord+gc).g;
	b1=COMPAT_TEXTURE(GUEST_S4,boord+bc).b;
	d=float3(r1,g1,b1);
...
float4 LuminancePS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float2 y2=2.0*dy;
	float ltotal=0.0;
	ltotal+=length( tex2Dlod(GUEST_S0,float4(float2(0.3,0.3),m,0)).rgb);
	ltotal+=length( tex2Dlod(GUEST_S0,float4(float2(0.3,0.7),m,0)).rgb);
	ltotal+=length( tex2Dlod(GUEST_S0,float4(float2(0.7,0.3),m,0)).rgb);
	ltotal+=length( tex2Dlod(GUEST_S0,float4(float2(0.7,0.7),m,0)).rgb);
	ltotal*=0.25;
	ltotal=pow(0.577350269*ltotal,0.7);
	float lhistory=tex2D(GUEST_S1,0.5).a;
	ltotal=lerp(ltotal,lhistory,lsmooth);
	float3 l1=COMPAT_TEXTURE(GUEST_S0,fuxcoord.xy   ).rgb;
	float3 r1=COMPAT_TEXTURE(GUEST_S0,fuxcoord.xy+dx).rgb;
	float3 l2=COMPAT_TEXTURE(GUEST_S0,fuxcoord.xy-dx).rgb;
	float3 r2=COMPAT_TEXTURE(GUEST_S0,fuxcoord.xy+x2).rgb;
	float c1=dist(l2,l1);
	float c2=dist(l1,r1);
	float c3=dist(r2,r1);
...
{
	float3 c1=tex2Dlod(GUEST_S0,float4(fuxcoord,0,0)).rgb;
	float3 c2=tex2Dlod(GUEST_S0,float4(fuxcoord+float2(0.0,OrgSize.w),0,0)).rgb;
...
float4 HGaussianPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=tex2Dlod(GUEST_S2, float4(tex+n*dx,0,0)).rgb;
	if(m_glow>0.5)
	{
...
float4 VGaussianPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=tex2Dlod(GUEST_S3, float4(tex+n*dy,0,0)).rgb;
	w=gauss_v(n+f);
	color=color+w*pixel;
	wsum=wsum+w;
	n=n+1.0;
...
float4 BloomHorzPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=tex2Dlod(GUEST_S2, float4(tex+n*dx,0,0));
	w=bloom_h(n+f);
...
float4 BloomVertPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=tex2Dlod(GUEST_S5, float4(tex+n*dy,0,0));
	w=bloom_v(n+f);
	pixel.a*=pixel.a*pixel.a;
	color=color+w*pixel;
	wsum=wsum+w;
...
{
	float2 prescalex=(tex2Dsize(GUEST_S2,0))/OrgSize.xy;
	float4 AdvSize=OrgSize*lerp(float4(prescalex.x,1.0,1.0/prescalex.x,1.0),float4(1.0,prescalex.y,1.0,1.0/prescalex.y),TATE);
	float gamma_in=1.0/COMPAT_TEXTURE(GUEST_S2,0.25).a;
	float lum=COMPAT_TEXTURE(GUEST_S1,0.5).a;
	float intera=COMPAT_TEXTURE(GUEST_S2,float2(0.75,0.25)).a;
	bool hscans=(hiscan>0.5);
	...
	{
	twl3=0.0;twr3=0.0;
	c1=tex2Dlod(GUEST_S1,float4(pC4     ,0,0)).xyz;
	c2=tex2Dlod(GUEST_S1,float4(pC4+offy,0,0)).xyz;
	c1=max(c1-sth,0.0);
	c2=max(c2-sth,0.0);
	}
	float3 l3,l2,l1,r1,r2,r3,color1,color2,colmin,colmax;
	l3=tex2Dlod(GUEST_S2,float4(pC4-off2     ,0,0)).rgb;
	l2=tex2Dlod(GUEST_S2,float4(pC4-offx     ,0,0)).rgb;
	l1=tex2Dlod(GUEST_S2,float4(pC4          ,0,0)).rgb;
	r1=tex2Dlod(GUEST_S2,float4(pC4+offx     ,0,0)).rgb;
	r2=tex2Dlod(GUEST_S2,float4(pC4+off2     ,0,0)).rgb;
	r3=tex2Dlod(GUEST_S2,float4(pC4+offx+off2,0,0)).rgb;
...
float4 ChromaticPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float2 pos=warp(lexcoord);
	float3 color=COMPAT_TEXTURE(GUEST_S7,pos1).rgb;
	float3 Bloom=COMPAT_TEXTURE(GUEST_S6,pos).rgb;
	float3 Glow=COMPAT_TEXTURE(GUEST_S4,pos).rgb;
	...
	float mx1=COMPAT_TEXTURE(GUEST_S7,pos1   ).a;
	...
	float mx0=tex2Dlod(GUEST_S7,float4(pos1-dx,0,0)).a;
	float mx2=tex2Dlod(GUEST_S7,float4(pos1+dx,0,0)).a;
	...
	mx0=tex2Dlod(GUEST_S7,float4(pos1-dx,0,0)).a;
	mx2=tex2Dlod(GUEST_S7,float4(pos1+dx,0,0)).a;

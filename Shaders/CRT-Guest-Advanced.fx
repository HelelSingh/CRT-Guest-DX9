...
float4 LuminancePS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float3 l1=COMPAT_TEXTURE(GUEST_S0,texcoord.xy   ).rgb;
	float3 r1=COMPAT_TEXTURE(GUEST_S0,texcoord.xy+dx).rgb;
	float3 l2=COMPAT_TEXTURE(GUEST_S0,texcoord.xy-dx).rgb;
	float3 r2=COMPAT_TEXTURE(GUEST_S0,texcoord.xy+x2).rgb;
	float c1=dist(l2,l1);
	float c2=dist(l1,r1);
	float c3=dist(r2,r1);
...
{
	float3 c1=tex2Dlod(GUEST_S0,float4(texcoord,0,0)).rgb;
	float3 c2=tex2Dlod(GUEST_S0,float4(texcoord+float2(0.0,OrgSize.w),0,0)).rgb;
